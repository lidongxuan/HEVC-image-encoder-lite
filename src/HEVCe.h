...
extern int HEVCImageEncoder (    // return    -1:error    positive value:HEVC stream length (in bytes)
unsigned char *pbuffer,          // buffer to save HEVC stream
unsigned char *img,              // 2-D array in 1-D buffer, height=ysz, width=xsz. Input the image to be compressed.
unsigned char *img_rcon,         // 2-D array in 1-D buffer, height=ysz, width=xsz. The HEVC encoder will save the reconstructed image here.
    ...
    const int  pmode_cand        // must be 1~35. The larger, the higher compression ratio, but the lower performance. The recommended value is 7.
);
